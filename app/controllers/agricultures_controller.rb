class AgriculturesController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @agriculture.save
        ...
        format.json { render action: 'show', status: :created, location: @agriculture }
      else
  ...
  def destroy
    @agriculture.destroy
    respond_to do |format|
      format.html { redirect_to agricultures_url }
      format.json { head :no_content }
    end
  end
  ...
  private
    ...
    def agriculture_params
      params.require(:agriculture).permit(:title, :content)
    end
end
